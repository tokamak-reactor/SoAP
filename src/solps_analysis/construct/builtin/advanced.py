...
def calc_pa(ne, te_eV, ti_eV, na, grid=None, comp=None, **kw):
    """Partial pressure per element.
    ...
    result = np.zeros((na.shape[0], max(n_elem, 1)))
    for ia in range(n_elem):
        ...
        p_neut = na[:, neutral_rows].sum(axis=1) * te_j if neutral_rows else 0
        p_ions = na[:, ion_rows].sum(axis=1) * ti_j if ion_rows else 0
...
def calc_zavg(na, grid=None, comp=None, **kw):
    """Average charge of each element."""
    n_elem = comp.n_elements if comp else 1
    result = np.zeros((na.shape[0], max(n_elem, 1)))
    for ia in range(n_elem):
        idx = comp.element_indices(comp.element_names[ia])
...
def calc_n_e_sep(ne, na, grid=None, comp=None, **kw):
    """n_e_sep[ia] = sum of charged states at separatrix / ne_avg at separatrix."""
    fcs = getattr(grid, 'core_sep_fcs', None)
    ...
    # Get separatrix cell pairs and average ne
    sep_ne = 0.0
    ...
    for fc in fcs:
        ...
        for ia in range(comp.n_elements):
            ...
            ion_rows = [i for i in idx if comp.zamax[i] > 0]
            if ion_rows:
                n_avg = sum(na[cv if cv < na.shape[0] else 0, i] for i in ion_rows for cv in [cv1, cv2]) / 2
...
def calc_total_particles(na, grid=None, comp=None, **kw):
    ...
    if comp is None:
        return np.array([(na[:n_core, :].sum(axis=1) * cv_vol[:n_core]).sum()])
    ...
    for ia in range(comp.n_elements):
        ...
        ion_rows = [i for i in idx if comp.zamax[i] > 0]
        if ion_rows:
            result[ia] = (na[:n_core, :][:, ion_rows].sum(axis=1) * cv_vol[:n_core]).sum()
    return result
